...
class PostGetter:
    ...
    def get_links(cls, rows: list):
        ...
        post_links = [cls.get_link(link) for link in links if 'mathdatasimplified' in cls.get_link(link)]
        code_links = [cls.get_link(link) for link in links if 'mathdatasimplified' not in cls.get_link(link)]
